...
//6) Обработка соединений от клиентов:
	...
	//dwError = WSAGetLastError();
	if (iResult == SOCKET_ERROR)cout << "Client shutdown failed with  " << WSAGetLastError() << endl;
	
	iResult = shutdown(client_socket, SD_BOTH);
	//dwError = WSAGetLastError();
	if (iResult == SOCKET_ERROR)cout << "Server shutdown failed with  " << WSAGetLastError() << endl;
